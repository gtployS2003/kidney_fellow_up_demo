/**
 * DonorLiffLayout — ฉีด Header + Bottom Nav สำหรับหน้า Donor LINE LIFF (มือถือ)
 * ใช้งาน: วางตำแหน่ง <div id="liff-header"></div> ... <div id="liff-bottomnav"></div>
 * เรียก DonorLiffLayout.init({ variant: 'home'|'plain', active, title, backHref })
 ...
 * - active: 'home'|'appointment'|'followup'|'chat'|'profile' -> แสดง Bottom Nav ไฮไลท์ตามค่านี้
...
(function () {
  const TABS = [
    ...
    { key: "followup", label: "ติดตามอาการ", icon: "activity", href: "followup.html" },
    { key: "chat", label: "แชท", icon: "message-circle", href: "chat.html" },
    { key: "profile", label: "โปรไฟล์", icon: "user", href: "profile.html" },
  ];

  function renderHeader({ variant, title, backHref }) {
    if (variant === "home") {
      const d = window.MOCK_DATA.primaryDonor;
      return `
      <header class="gradient-mesh sticky top-0 z-20 px-5 pt-6 pb-8 text-white" style="background: var(--gradient-primary);">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-[11px] text-blue-100 mt-0.5">รหัสผู้บริจาค: ${d.id} · ${d.hospital}</p>
          </div>
          <div class="flex items-center gap-2">
            ...
            <a href="profile.html" class="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-xs font-bold liff-tap">${d.name.slice(0, 2)}</a>
          </div>
        </div>
      </header>`;
    }
